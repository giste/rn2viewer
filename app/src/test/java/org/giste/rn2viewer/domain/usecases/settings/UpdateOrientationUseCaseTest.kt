...
class UpdateOrientationUseCaseTest {
    ...
    private val updateOrientationUseCase = UpdateOrientationUseCase(repository)

    @Test
    fun `invoke should call setOrientation on repository`() = runTest {
        val orientation = AppOrientation.HORIZONTAL
        coEvery { repository.setOrientation(orientation) } returns Unit

        updateOrientationUseCase(orientation)

        coVerify { repository.setOrientation(orientation) }
    }
}
